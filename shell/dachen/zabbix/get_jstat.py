...
jstat_cmd = "/data/program/jdk8/bin/jstat"
#jvmname_cmd = "netstat -tpnl|grep -oP '(?<=:)\d+.*\d+(?=/java)'|awk '{print $1,$NF}'"
jvmname_cmd = "ps -ef | grep -v grep | grep war | awk '{print $24,$2}' |  cut -d '/' -f 5 "
#hostname = socket.gethostname()
hostname = commands.getoutput("ifconfig |grep 'inet'|grep '172' | grep -v 'grep' | awk '{print $2}'")
#hostname = "pre"
...
def get_jmx(jname,jprocess):
    ...
    for jmxkey in data_dict.keys():
        if jmxkey in jstat_dict.keys():
            ...
            zbx_data = "%s jstat[%s,%s] %s" %(hostname,jname,cur_key,data_dict[jmxkey])
...
def zbx_tmp_file_create():
    ...
    for jvm_tmp in jvmname_list:
        ...
        th = threading.Thread(target=get_jmx,args=(jvmname,jvmprocess))
        th.start()
...
def zbx_discovery():
    ...
    jvmname_list = jvm_port_discovery()
    for jvm_tmp in jvmname_list:
        jvm_zabbix.append({'{#JNAME}' : jvm_tmp[0],'{#JPROCESS}' : jvm_tmp[1],})
    #return json.dumps({'data': jvm_zabbix}, sort_keys=True, indent=7,separators=(',', ':'))
    return json.dumps({'data': jvm_zabbix})    
...
def cmd_line_opts(arg=None):
    ...
    parse.add_argument('--jvmname', action='store_true', help='获取JVM端口')
